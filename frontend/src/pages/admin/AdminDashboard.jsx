  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      try {
        ...
        setQuizPerformance(response.data.quizPerformance || []);
  ...
  return (
    <AdminLayout>
      ...
      >
        ...
        <DashboardCard title="Total Quizzes" value={statistics.total_quizzes} />
        ...
        <DashboardCard title="Draft Quizzes" value={statistics.draft_quizzes} />
        ...
        <DashboardCard
          title="Average Score"
          value={`${Number(statistics.average_score).toFixed(2)}%`}
        />
      ...
      <div className="admin-analytics-grid">
        ...
        <section className="admin-analytics-card">
          ...
          <p>Average student performance for each quiz.</p>

          {quizPerformance.length === 0 ? (
            <p>No quiz performance data available.</p>
          ) : (
            quizPerformance.map((quiz) => {
              const percentage = Number(quiz.average_score) || 0;

              return (
                <div key={quiz.quiz_id} className="admin-analytics-item">
                  <div className="admin-analytics-item-top">
                    <div>
                  ...
                  <div className="admin-analytics-meta">
                    <span>Passed: {quiz.passed_attempts}</span>

                    <span>Failed: {quiz.failed_attempts}</span>
                  </div>
                </div>
              );
            })
        ...
        <section className="admin-analytics-card">
          ...
          <p>Average scores across quiz categories.</p>
